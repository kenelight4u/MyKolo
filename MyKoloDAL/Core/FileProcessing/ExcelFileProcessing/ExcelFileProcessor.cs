...
namespace MyKoloDAL.Core.FileProcessing.ExcelFileProcessing
{
    public class ExcelFileProcessor : FileProcessorBase, IFileProcessor
    {
        ...
        public bool WriteToFile(List<SingleFieldMeta> convertedSingleRecordData)
        {
            ...
            switch (convertedSingleRecordData[0].ColumnValue.ToString())
            {
                case "Expenses":
            ...
            for (int index = 0; index <= convertedSingleRecordData.Count; index++)
            {
                ...
                targetRange.Value = targetField.ColumnValue;
            }
